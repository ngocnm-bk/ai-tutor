...
|---|---|---|
| **Ingest** | `src/ai_tutor/ingest/` | Xử lý file theo loại: ảnh→**Claude vision**, pdf/docx→text, video→`ffmpeg`+**faster-whisper**, text→thẳng. Idempotent (hash chống trùng), lỗi đẩy vào `inbox/_failed/` không chặn file khác. |
| **Classify** | `src/ai_tutor/classify/` | Claude (structured output JSON) gắn `lop/mon/chuong/bai/ky_nang`. |
...
`python-telegram-bot` · `anthropic` (Claude) · `faster-whisper` + ffmpeg · `pypdf` / `python-docx` · `sentence-transformers` + `chromadb` (Phase 2) · `sqlite3` (stdlib).
...
Dùng model Claude mới nhất khi gọi API (tutor, phân loại, đọc ảnh). Ảnh đọc bằng **Claude vision** — không thêm engine OCR riêng. Phân loại dùng **structured output** để ép JSON đúng schema. Trong unit test, **mock Claude API** (không gọi mạng).
